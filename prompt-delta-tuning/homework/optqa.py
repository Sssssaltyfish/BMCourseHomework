...
parser.add_argument(
    "--modified_modules", type=str, nargs="+", default=["self_attn", "fc2"]
)
...
parser.add_argument(
    "--mode", type=str, default="train", choices=["train", "interactive_inference"]
)
...
def get_dataset(args):
    ...
    datasets = load_dataset(path, name=os.path.basename(path))
    return {
        name: [
            InputExample(
                guid=data["question_id"],
...
def add_delta_to_backbone(plm, args):
    ...
    #############################################################
    print("before delta:")
    opendelta.Visualization(plm).structure_graph()
    config = opendelta.AutoDeltaConfig.from_dict(dict(args._get_kwargs()))
    delta_model = opendelta.AutoDeltaModel.from_config(config, plm)
    delta_model.freeze_module(plm)
    print("after delta:")
    opendelta.Visualization(plm).structure_graph()
    print("delta:")
    opendelta.Visualization(delta_model).structure_graph()
    return delta_model
...
def load_from_finetuned(plm, args):
    ...
    ##
    #############################################################
    delta_model = opendelta.AutoDeltaModel.from_finetuned(args.delta_save_path, plm)
    return delta_model
...
def get_prompt_template(tokenizer, args):
    ...
    ##
    #############################################################
    mytemplate = ManualTemplate(tokenizer=tokenizer, text='The answer is {"mask"}')
    return mytemplate
...
def main(args):
    ...
    myverbalizer = GenerationVerbalizer(
        tokenizer, classes=None, label_words={0: ["{'meta':'answer'}"]}, is_rule=True
    )
    ...
    ##
    #####################################################################
    optimizer = torch.optim.Adam(prompt_model.parameters(), lr=args.lr)
    ...
    for epoch in range(args.train_epochs):
        ...
        val_f1 = evaluate(
            prompt_model, validation_dataloader, args
        )  # is slow due to generation
    ...
    print(f"performance on test: {test_f1}")

    if args.push_to_dc:
        delta_model.save_finetuned(
            f"{args.delta_save_path}/",
            push_to_dc=True,
            center_args={
                "name": f"{args.plm_path.split('/')[-1]}-{args.delta_type}-for-direct-qa",
...
def interactive_inference(args):
    ...
    while True:
        ...
        inference_dataloader = PromptDataLoader(
